...
class Autoencoder_Trainer(object):
    ...
    def init_dataset(self, path_prefix = ""):
        ...
        trainset = torchvision.datasets.MNIST(root='{}'.format(path_prefix),  train=True,download=True, transform=transform)
        train_loader = torch.utils.data.DataLoader(trainset, batch_size=32, shuffle=False, num_workers=0)
        valset = torchvision.datasets.MNIST(root='{}'.format(path_prefix), train=False, download=True, transform=transform)
